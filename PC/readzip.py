...
def select(sql):
	try:
		conn=connect(host='192.168.1.198',user='root',passwd='123123',db='cleanpt',port=3306,charset='utf8')
		cur=conn.cursor()
		cur.execute(sql)
		data=cur.fetchall()
		return data
...
def update(sql):
	conn=connect(host='localhost',user='root',passwd='123123',db='pc',port=3306,charset='utf8')
	cur=conn.cursor()
	if cur.execute(sql):
		conn.commit()
		return 1
...
def findtask(waittime):
	...
	if(len(data)>0):
		...
		if flag1==1:
			...
			print('大数据平台开始处理')
			if zf[-4:]!='.zip':
				shutil.copyfile(str(data[0][4]),'/home/cph/'+zf)
				#shutil.move(str(data[0][4]),'/home/cph/')
				print('上传文件是'+str(data[0][4]))
				zfexist=addzip2(file=zf,path=zp)
				if zfexist!=0:
					zf=zfexist
			print(zf)
...
def start(i=0):
	while 1:
		try:
			...
			undo=update('update taskinfo set tasklevel=-1 where taskstatus=1')
			if undo==1:
